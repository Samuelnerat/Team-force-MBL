...
const ColoredCards = ({ icon, title, variant = 'default', className = '' }) => {
  ...
  return (
    <article className={`relative w-full h-[200px] lg:h-[280px] group ${className}`}>
      <div
        className={`absolute inset-0 transform -rotate-12 group-hover:rotate-0 transition-transform duration-500 ${bgGradient} rounded-2xl w-full h-full shadow-stackedCard`}
      />
      <div
